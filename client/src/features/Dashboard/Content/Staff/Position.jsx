...
import Add_position from './Add_position';
...
const All = () => {
    ...
    const { jobLoading, jobs } = useSelector(jobSelector);
    ...
    const handleDeleteJob = (id) => {
        dispatch(Array.isArray(id) ? deleteMultiJob(id) : deleteJob(id)).then((data) => {
            dispatch(getAll())
    ...
    if (jobs) {
        if(jobs.length > 0) {
            data = jobs.map((job) => {
                const staff = staffs.filter((index) => index.position._id === job._id);
                return (
                    <tr key={job._id}>
                        <td>
                            <input
                        ...
                        <td>
                            {showDelete ? 
                                <button title='Restore' className={styles.restoreBtn} onClick={() => handleRestoreJob(job._id)}>Restore</button> :
                                <>
                                    <i
                                        title='Edit'
                                        className='fas fa-edit'
                                        onClick={() => setEdit({ show: true, job })}
                                    ></i>
                                    <i
                                        title='Delete'
                                        className='fas fa-trash-alt'
                                        onClick={() => setDeleteConfirm({show:true, data: job._id})}
                                    ></i>
                                </>
                            }
                        </td>
                ...
                {edit.show && (
                    <div className={styles.modal} onClick={(e) => closeModal(e)}>
                        <div className={clsx(styles.modalContainer, theme === "light" ? 'shadow_light' : 'shadow')}>
                            ...
                            <Add_position job={edit.job} setEdit={setEdit} setMessagesAll={setMessages} edit={true} />
                        </div>
                    </div>
                )}
                {deleteConfirm.show && <DeleteConfirm deleteAction={handleDeleteJob}/>}
                <div className={styles.allHeader}>
                    <div className={styles.headerLeft}>
                        <div className={styles.allName}>Position</div>
                        <div title='Search' className={styles.allSearch}>
                            <i className='fas fa-search'></i>
                            <input type='text' placeholder='Search' value={search} onChange={(e) => setSearch(e.target.value)} />
                        </div>
                        { itemChoose.length > 0 ? showDelete ?
                            <div title='Delete' className={styles.icon} onClick={() => handleRestoreJob(itemChoose)}>
                    ...
                    <div className={styles.headerRight}>
                        <div
                            ...
                            onClick={() => setEdit({ show: true, job: null })}
                        >
